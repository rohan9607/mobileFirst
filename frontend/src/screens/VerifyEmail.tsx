"use client"
import { Button } from '@/components/utilityComponents/button/Button'
import { useAppDispatch } from '@/redux/hooks'
import { verifyEmail } from '@/redux/thunks/auth'
import Link from 'next/link'
import { useRouter, useSearchParams } from 'next/navigation'
import React, { useEffect, useState } from 'react'

type Props = {}

const VerifyEmail = (props: Props) => {
    const params: any = useSearchParams()
    const dispatch = useAppDispatch()
    const router = useRouter()
    const [loading, setIsLoading] = useState<boolean>(false);
    const [resData, setResData] = useState(null);
    const [isError, setIsError] = useState<boolean>(false);
    const token: string = params.get('token')
    const email: string = params.get('email')

    useEffect(() => {
        if (email !== null && token !== null) {
            setIsLoading(true)
            dispatch(verifyEmail({ token, email }))
                .unwrap()
                .then((res: any) => {
                    console.log({res});
                    
                    if (res?.success) {
                        setResData(res.data)
                        setIsLoading(false)
                    }
                    else {
                        setIsError(true)
                        setIsLoading(false)
                    }
                })
        }
    }, [params])

    
    return (
        <div className='flex flex-col justify-center space-y-5'>
            {
                token === null &&
                <>
                <div className='flex flex-row'>
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ff1a1a" className="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
                    </svg>
                <h1 className='ml-3'>Something went wrong</h1>
                </div>
                <Link href={"/"} className='text-blue-400'>go_to_home</Link>
                </>
            }
            {loading &&
                <div role="status" className='flex flex-row align-middle justify-between'>
                    <svg aria-hidden="true" className="inline w-8 h-8 mr-2 text-gray-200 animate-spin dark:text-gray-600 fill-red-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
                        <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill" />
                    </svg>
                    <span className="sr-only">Loading...</span>
                    <div>
                        <h5>Verifying your email, Please wait for moment</h5>
                    </div>
                </div>
            }
            {resData !== null && !isError &&
                <>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#5cd6d6" className="w-20 h-w-20 mx-auto">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12c0 1.268-.63 2.39-1.593 3.068a3.745 3.745 0 01-1.043 3.296 3.745 3.745 0 01-3.296 1.043A3.745 3.745 0 0112 21c-1.268 0-2.39-.63-3.068-1.593a3.746 3.746 0 01-3.296-1.043 3.745 3.745 0 01-1.043-3.296A3.745 3.745 0 013 12c0-1.268.63-2.39 1.593-3.068a3.745 3.745 0 011.043-3.296 3.746 3.746 0 013.296-1.043A3.746 3.746 0 0112 3c1.268 0 2.39.63 3.068 1.593a3.746 3.746 0 013.296 1.043 3.746 3.746 0 011.043 3.296A3.745 3.745 0 0121 12z" />
                    </svg>
                    <h1 className='mx-auto'>Email has been verified</h1>
                    <Button onClick={() => {router.push("/login")}} className=' w-60'>Login To Dashboard</Button>
                </>
            }
            {
                isError &&
                <>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="#ff1a1a" className="w-6 h-6">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 11-18 0 9 9 0 0118 0zm-9 3.75h.008v.008H12v-.008z" />
                    </svg>
                    <h1 className='mx-auto'>Something went wrong! Email not verified</h1>
                </>
            }
        </div>
    )
}

export default VerifyEmail